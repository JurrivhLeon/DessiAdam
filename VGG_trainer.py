...
Optimization.
Junyi Liao
Train VGG networks over CIFAR-10.
"""
...
def train(
    ...
    for epoch in range(epochs_n):
        ...
        filter_norms.append(
            model.state_dict()['features.18.weight'].view(512, -1).norm(dim=-1)
        )
...
if __name__ == '__main__':
    ...
    if bn:
        ...
        ckpt_dir = f'./checkpoints/VGG_A_BN_DessiLBI1e1'
    else:
        model = VGG_A().to(device)
        ckpt_dir = f'./checkpoints/VGG_A'
    ...
    # optimizer = SLBI(model.parameters(), lr=lr, kappa=1, mu=100)
